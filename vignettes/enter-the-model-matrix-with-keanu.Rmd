...
fit1$resid_plot <- augment(fit1) %>%
  ...
  theme_strong() +
  ggtitle("Residuals") + theme(plot.title = element_text(hjust = 0.5))

fit1$coef_plot <- plot_coefficients(fit1) +
  theme_strong(rotate_xlab=TRUE) +
  scale_x_discrete("") + scale_y_continuous("Estimate") + 
  ggtitle("Coefficients") + theme(plot.title = element_text(hjust = 0.5))

fit1$resid_plot + fit1$coef_plot + 
  plot_layout(ncol = 1) + 
  plot_annotation("First Model Attempt", theme = theme(plot.title=element_text(size=18)))
```
...
fit2$resid_plot + fit2$coef_plot + 
  plot_layout(ncol = 1) + 
  plot_annotation("Second Model Attempt", theme = theme(plot.title=element_text(size=18)))
```
...
plot_coefficients(fit3, terms = vars(starts_with("time_of_day"), session_length)) +
  ...
  ggtitle("Lognormal Model — Coefficients")
```
